...
# compressed=True
def gen_eckey(passphrase=None, secret=None, pkey=None, compressed=None, rounds=1, version=0):
    k = KEY()
    if passphrase:
...
def get_addr(k,version=0):
    ...
    return addr, pkey, secret.encode("hex")
...
def test(otherversion):
    ...
    shift=  6 # Caracteres de 1 a 64 de la clave privada anterior
    frase = "frasesecreta" # Frase que se antepone a los caracteres seleccionados en shift
    ...
    for i in range(51):
        ...
        key = gen_eckey(passphrase=phrase,compressed=True)
        a = get_addr(key,version=otherversion)

    # otherversion=58 QUARKCOIN , 48 LITECOIN  y 0 BITCOIN
    # para bitcoin se suele usar el formato DESCOMPRIMIDO y para QUARKCOIN y otras COMPRIMIDO

        linea = a[0] + "," + a[1] + "," + phrase
        #print a[0]
        #print a[1]
        #print a[2][:shift],a[2]
        if i > 0 :
        ...
        phrase = frase + a[2][:shift] # La frase que genera la clave privada de la siguiente
    ...
    # by passphrase
    #a = (gen_eckey(passphrase='Max Guevara').get_secret())a.encode("hex")
